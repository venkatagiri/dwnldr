...
module.exports = {
  ...
  getFormats: function(url, callback) {
    execute([url, '-F'], function(error, result) {
      if(!!error) {
        console.error('ytdl.getFormats: error(', error, ')');
        if(error.search('HTTP Error 404') !== -1) return callback('Video not found!');
  ...
  getDownloadLink: function(url, format, callback) {
    execute([url, '-f', format, '--get-url'], function(error, result) {
      if(!!error) {
        console.error('ytdl.getDownloadLink: error(', error, ')');
        if(error.search('HTTP Error 404') !== -1) return callback('Video not found!');
        if(error.search('requested format not available') !== -1) return callback('Requested format not available!');
        return callback('Cound not find any download links!');
      }
