...
function execute(args, callback) {
  execFile(youtube_dl, args, {shell: true}, function(error, stdout, stderr){
...
module.exports = {
  ...
  getFormats: function(url, callback) {
    execute([url, '--verbose', '-F'], function(error, stderr, result) {
      if(!!error) {
  ...
  getDownloadLink: function(url, format, callback) {
    execute([url, '--verbose', '-f', format, '--get-url'], function(error, stderr, result) {
      if(!!error) {
        if(stderr.search('HTTP Error 404') !== -1) return callback('Video not found!');
